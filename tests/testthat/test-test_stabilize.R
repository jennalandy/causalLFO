test_that("stabilize works", {
  ...
  res <- stabilize(M, Tr, rank = 3, reference_P = true_P)
  expect_equal(length(res$ATE), 3)
  expect_equal(dim(res$Phat), c(96, 3))
  expect_equal(dim(res$Chat), c(3, 10))
})

test_that("bootstrap_wrapper of stabilize works called internally", {
  true_P = matrix(rexp(96*3, rate = 1), nrow = 96)
  true_P = sweep(true_P, 2, colSums(true_P), '/')
  true_C = matrix(rexp(3*10, rate = 0.01), nrow = 3)
  Tr = sample(c(0, 1), 10, replace = TRUE)
  ...
  res <- stabilize(
    ...
    bootstrap = TRUE, bootstrap_file = "stabilize",
    bootstrap_reps = 10
  )
  ...
  expect_equal(dim(res$Phat), c(96, 3))
  expect_equal(dim(res$Chat), c(3, 10))
  expect_equal(res$est_file, "stabilize.csv")
  expect_equal(res$all_Ps_file, "stabilize_aligned_Ps.rds")
